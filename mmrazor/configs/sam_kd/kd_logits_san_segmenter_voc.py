...
model = dict(
    ...
    distiller=dict(
        ...
        distill_losses=dict(
            loss_kd_before_proj=dict(type='KLDivergence', tau=1, loss_weight=batch_size, reduction='mean'),
            # loss_kd_after_proj=dict(type='KLDivergence', tau=1, loss_weight=3, reduction='mean')
        ),
        student_recorders=dict(
            logits_wo_projection=dict(type='ModuleOutputs', source='decode_head.mask_result_module'),
        ...
        loss_forward_mappings=dict(
            loss_kd_before_proj=dict(
                preds_S=dict(from_student=True, recorder='logits_wo_projection'),
                preds_T=dict(from_student=False, recorder='logits')),
            # loss_kd_after_proj=dict(
            #     s_feature=dict(from_student=True, recorder='logits_w_projection'),
            #     t_feature=dict(from_student=False, recorder='logits'))
        )
    )
)
