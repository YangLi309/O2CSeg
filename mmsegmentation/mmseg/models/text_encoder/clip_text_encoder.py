...
@MODELS.register_module()
class LearnablePromptCLIPTextEncoder(BaseModule):
    """A text encoder with transformer architecture to encode the label text.
    ...
    def __init__(self,
                 ...
                 is_context_prompt_learnable: bool = False,
                 is_class_specific: bool = False,
                 templates: str = 'vild',
                 total_vocab_size: int = 49408,
                 context_length: int = 77,
                 embed_dims: int = 512,
        ...
        self.is_context_prompt_learnable = is_context_prompt_learnable
        self.tokenized = None
        self.register_buffer(
            'attn_mask', self.build_attention_mask(), persistent=False)
        for i in range(num_layers):
        ...
        if self.is_class_specific:
            self.context_embeddings = nn.Parameter(torch.randn(len(vocabulary), num_contexts, embed_dims) * 0.02)
        else:
            self.context_embeddings = nn.Parameter(torch.randn(num_contexts, embed_dims) * 0.02)
    ...
    def init_weights(self):
        nn.init.normal_(
            ...
            std=self.context_embeddings.shape[1] ** -0.5,
        )
    ...
    def init_token_prefix_suffix(self):
        print('init_token_prefix_suffix')
        if self.dataset_name is None:  # encoding vocabulary directly
            class_names = self.vocabulary
            self.template_encode(class_names)
        else:  # encoding the classes of the dataset
            class_names = get_classes(self.dataset_name)
            if class_names[0] == 'background':
                class_names = class_names[1:]
    ...
    def encode_text(self, text, normalize=False):
        """encode class token."""
        x = self.token_embedding(text.to(get_device()))  # [batch_size, n_ctx, d_model]
        self.register_buffer("token_prefix", x[:, :1, :])  # SOS
        self.register_buffer("token_suffix", x[:, 1 + self.num_contexts:, :])
        self.tokenized_argmax = self.tokenized.argmax(dim=-1).detach().clone()
    ...
    def template_encode(self, vocabulary):
        """Prompt engineering."""
        prompt_prefix = " ".join(["X"] * self.num_contexts)
        prompt_prefix += " {}."
        template = prompt_prefix
        text_inputs = tokenizer.tokenize(
            [template.format(noun) for noun in vocabulary])
        self.tokenized = text_inputs
        self.encode_text(self.tokenized, normalize=True)

    def forward(self):
        if not hasattr(self, 'token_prefix'):
            self.init_token_prefix_suffix()
        # print(torch.max(self.context_embeddings))
        # print(self.context_embeddings.requires_grad)
        """Forward function."""
        if self.context_embeddings.dim() == 2:
            context_emb = self.context_embeddings.unsqueeze(0).expand(len(self.vocabulary), -1, -1).contiguous()
            prompt_embs = torch.cat([self.token_prefix, context_emb, self.token_suffix], dim=1)
        else:
            prompt_embs = torch.cat([self.token_prefix, self.context_embeddings, self.token_suffix], dim=1)
        x = prompt_embs.to(get_device())
        x = x + self.positional_embedding
        x = x.permute(1, 0, 2)  # NLD -> LND
        for block in self.transformer:
            x = block(query=x, attn_masks=self.attn_mask)
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x)  # [batch_size, n_ctx, transformer.width]
        x = x[torch.arange(x.shape[0]), self.tokenized_argmax] @ self.text_projection
        class_embeds = F.normalize(x, dim=-1)
