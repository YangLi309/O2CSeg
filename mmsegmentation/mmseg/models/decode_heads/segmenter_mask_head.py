...
class SegmenterMaskTransformerHead(BaseDecodeHead):
    ...
    def __init__(
        ...
        self.mask_after_projection_result_module = nn.Identity()
        self.kd_training = kd_training
        if self.kd_training:
            self.projector = nn.Linear(self.num_classes, self.num_classes)
    ...
    def forward(self, inputs):
        ...
        if self.kd_training:
            mask_logits = self.predict_by_feat(masks, [{'img_shape': self.crop_size}])
            non_projected_output = self.mask_result_module(mask_logits)
            projected_student_logits = self.projector(mask_logits.transpose(1, 3)).transpose(1, 3)
            projected_output = self.mask_after_projection_result_module(projected_student_logits)
        return masks
