...
class BaseDecodeHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        if self.kd_training:
            ...
            self.sample_weight_module = nn.Identity()
    ...
    def loss_by_feat(self, seg_logits: Tensor,
        ...
        if self.kd_training:
            self.mask_result_module(seg_logits)
            if self.sampler is not None:
                self.sample_weight_module(seg_weight)
    ...
    def predict_by_feat(self, seg_logits: Tensor,
        ...
        #     mask_logits = torch.mean(seg_logits, dim=1)
        #     plt.figure(figsize=(10, 10))
        #     plt.imshow(mask_logits.squeeze().cpu().numpy(), cmap='viridis',
        #                interpolation='nearest',
        #                alpha=0.9)
        #     plt.axis('off')
        #     import random
        #     plt.savefig(os.path.join(f"{filename_without_extension}_segmenter_logits_{str(random.randint(1, 100))}.png"),
